...
export default function DeanBoardExam() {
  ...
  const handleApprove = async (id) => {
    await api.put(`/board-exam-results/${id}`, { status: "approved", approved_by: user.id, approved_at: new Date().toISOString() });
    loadData();
  };

  const handleReject = async (id) => {
    await api.put(`/board-exam-results/${id}`, { status: "rejected", rejection_reason: rejectReason });
    setRejectId(null);
    setRejectReason("");
    loadData();
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-[#7b1113] rounded-2xl p-6 text-white">
        <h1 className="text-2xl font-bold">Board Exam Submissions 📝</h1>
        <p className="text-red-200 text-sm mt-1">Review and approve board exam results.</p>
      </div>
