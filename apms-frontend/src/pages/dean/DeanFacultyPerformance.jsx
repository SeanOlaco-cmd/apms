...
export default function DeanFacultyPerformance() {
  ...
  const handleApprove = async (id) => {
    await api.put(`/faculty-performance/${id}/review`, { status: "approved" });
    loadData();
  };

  const handleReject = async (id) => {
    await api.put(`/faculty-performance/${id}/review`, { status: "rejected", rejection_reason: rejectReason });
    setRejectId(null);
    setRejectReason("");
    loadData();
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-[#7b1113] rounded-2xl p-6 text-white">
        <h1 className="text-2xl font-bold">Faculty Performance Submissions 👨‍🏫</h1>
        <p className="text-red-200 text-sm mt-1">Review and approve faculty performance data from your department heads.</p>
      </div>
