...
export default function DeanEmployees() {
  ...
  const handleApprove = async (id) => {
    await api.put(`/employees/${id}/review`, { status: "approved" });
    loadData();
  };

  const handleReject = async (id) => {
    await api.put(`/employees/${id}/review`, { status: "rejected", rejection_reason: rejectReason });
    setRejectId(null);
    setRejectReason("");
    loadData();
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-[#7b1113] rounded-2xl p-6 text-white">
        <h1 className="text-2xl font-bold">Employee Submissions 🧑‍💼</h1>
        <p className="text-red-200 text-sm mt-1">Review and approve part-time/full-time employee data.</p>
      </div>
