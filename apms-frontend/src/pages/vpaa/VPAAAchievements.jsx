...
export default function VPAAAchievements() {
  ...
  const handleApprove = async (id) => {
    await api.put(`/achievements/${id}`, { status: "approved" });
    loadData();
  };

  const handleReject = async (id) => {
    await api.put(`/achievements/${id}`, { status: "rejected", rejection_reason: rejectReason });
    setRejectId(null);
    setRejectReason("");
    loadData();
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-[#7b1113] rounded-2xl p-6 text-white">
        ...
        <p className="text-red-200 text-sm mt-1">Review and approve achievement records submitted by Deans.</p>
      </div>
